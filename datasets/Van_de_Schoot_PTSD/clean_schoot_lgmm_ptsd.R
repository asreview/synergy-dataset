...
#original papers
setwd("csv")
all.data <- read_csv("schoot-lgmm-ptsd-initial.csv")

sum(!is.na(all.data$abstract) & !is.na(all.data$title))


#included papers
inc.data <- read_csv("schoot-lgmm-ptsd-included-2.csv")

#After abstract screening
aas.data <- read_csv("schoot-lgmm-ptsd-included-1.csv")


# inc.data$title[! inc.data$title %in% all.data$title]
...
cat("\nPapers included, missing from initial data:            ", sum(! tolower(inc.title) %in% tolower(all.title)), "\n")
...
# print(unique.train.data$title[paper_direct])
